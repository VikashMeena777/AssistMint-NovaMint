...
const TIER_CONFIG = {
  bronze: { label: "Bronze", color: "text-warning", bg: "bg-warning/10", border: "border-warning/30", emoji: "🥉", threshold: 0 },
  silver: { label: "Silver", color: "text-muted-foreground", bg: "bg-muted", border: "border-border", emoji: "🥈", threshold: 500 },
  gold: { label: "Gold", color: "text-seal", bg: "bg-seal/10", border: "border-seal/30", emoji: "🥇", threshold: 2000 },
  platinum: { label: "Platinum", color: "text-primary", bg: "bg-primary/10", border: "border-primary/30", emoji: "💎", threshold: 5000 },
} as const;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Loyalty & Rewards</h1>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className={`text-2xl font-bold font-mono tabular-nums ${cfg.color}`}>{count}</span>
              </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="rounded-xl border border-border/50 bg-card p-4">
          ...
          <p className="text-2xl font-bold font-mono tabular-nums text-foreground">{(stats.totalEarned || 0).toLocaleString()}</p>
          <p className="text-xs text-muted-foreground">Points Issued</p>
        </div>
        <div className="rounded-xl border border-border/50 bg-card p-4">
          <div className="flex items-center gap-2 mb-2"><Gift className="h-4 w-4 text-success" /></div>
          <p className="text-2xl font-bold font-mono tabular-nums text-success">{(stats.totalRedeemed || 0).toLocaleString()}</p>
        ...
        <div className="rounded-xl border border-border/50 bg-card p-4">
          ...
          <p className="text-2xl font-bold font-mono tabular-nums text-primary">{(stats.totalPoints || 0).toLocaleString()}</p>
          <p className="text-xs text-muted-foreground">Points in Circulation</p>
        </div>
        <div className="rounded-xl border border-border/50 bg-card p-4">
          <div className="flex items-center gap-2 mb-2"><Users className="h-4 w-4 text-warning" /></div>
          <p className="text-2xl font-bold font-mono tabular-nums text-warning">{stats.activeMembers || 0}</p>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1 rounded-lg bg-warning/10 px-2.5 py-1 border border-warning/25">
                      <Star className="h-3 w-3 fill-warning text-warning" />
                      <span className="text-xs font-bold font-mono tabular-nums text-warning">{reward.points_required}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-2 pt-2 border-t border-border/40">
                    ...
                    <button
                      ...
                      className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-destructive/25 text-destructive hover:bg-destructive/10 transition-all"
                    >
          ...
          <div className="rounded-2xl border border-border/50 bg-card overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {transactions.map((tx) => (
                    <tr key={tx.id} className="border-b border-border/30 last:border-0 hover:bg-secondary/60">
